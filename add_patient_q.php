...
// Get recent queues for the current user (last 15 minutes)
$recentQueues = [];
$stmt = $conn->prepare("SELECT q.*, d.name as department_name 
                       FROM queues q
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $sql = "INSERT INTO queues (queue_num, service_name, department_id, status, priority, created_at, created_by)
                VALUES (:queue_num, :service_name, :department_id, 'waiting', :priority, NOW(), :created_by)";
        ...
        $deptName = $conn->prepare("SELECT name FROM departments WHERE dept_id = ?");
        $deptName->execute([$department_id]);
        $deptName = $deptName->fetchColumn();
        ...
        $error_message = "Error adding to queue: " . $e->getMessage();
    }
}
?>
            ...
            <div class="recent-queues">
                ...
                <div class="list-group">
                    <?php foreach ($recentQueues as $queue): ?>
                        <a href="display_ticket.php?reprint=<?= $queue['qid'] ?>" 
                           class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                            <div>
                                <strong><?= htmlspecialchars($queue['queue_num']) ?></strong>
                                <div class="text-muted small mt-1">
                                    <?= htmlspecialchars($queue['department_name']) ?> - 
                                    <?= htmlspecialchars($queue['service_name']) ?>
                                </div>
                            </div>
                            <div class="text-end">
                                <span class="badge queue-badge bg-<?= 
                                    ($queue['priority'] == 'high') ? 'danger' : (($queue['priority'] == 'medium') ? 'warning' : 'secondary');
                                ?>">
                                    <?= ucfirst($queue['priority']) ?>
                                </span>
                                <div class="text-muted small mt-1">
                                    <?= date('h:i A', strtotime($queue['created_at'])) ?>
                                </div>
                            </div>
                        </a>
                    <?php endforeach; ?>
